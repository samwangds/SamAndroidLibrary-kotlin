 ...
 *         2016/1/15 14:26
 */
class DefaultClickEffectTranslucence(val interpolator: TimeInterpolator = DecelerateInterpolator()) : ViewClickEffect{
    override fun onPressedEffect(view: View) {
        view.animate().alpha(scale).setDuration(duration).interpolator = interpolator
    }

    override fun onUnPressedEffect(view: View) {
        view.animate().alpha(1f).interpolator = interpolator
    }
